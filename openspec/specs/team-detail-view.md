...
(Previously: an English `<ul>` from `computeCoachingCostItems` with an optional Apothecary line, compact `k` values — later a table with fixed `px-[10px]` cell padding and no horizontal-scroll wrapper.)
