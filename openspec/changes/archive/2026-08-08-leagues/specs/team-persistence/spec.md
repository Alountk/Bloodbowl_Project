...
### Requirement: League-Active Teams Not Archivable (Future Invariant)

(Reason: leagues now exist, so the deferred invariant is enforced directly by the Archive Guard here and the "Deletion blocked for league member" scenario — a member team's DELETE returns 409.)
(Migration: the invariant is superseded by the enforced 409 guard on `/api/teams/[id]` DELETE; no external consumer exists.)
