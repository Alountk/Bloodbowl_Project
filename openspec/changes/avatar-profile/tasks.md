...
- [ ] 3.2 `components/UserAvatar.tsx`: `<img>` if `src` else nothing; test via `textContent`/regex (no jest-dom)
- [ ] 3.3 `features/profile/api.ts` (getMe/patchMe/uploadAvatar) + `CropDialog.tsx` (aspect 1, pan+zoom)
- [ ] 3.4 `app/profile/page.tsx` Spanish copy; show avatar, upload control, preview updates
