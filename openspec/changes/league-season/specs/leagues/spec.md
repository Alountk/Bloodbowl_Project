...
(Previously: League had id/name/description/ownerId/createdAt only; delete always cleared members and removed the row.)
...
(Previously: list scoped strictly to the owner and detail was owner-only.)
...
(Previously: join required the league to be owned by the session user, i.e. owner-scoped only, with no status lock.)
