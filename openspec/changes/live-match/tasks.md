...
- [ ] 2.1 Modify `features/leagues/api.ts`: add `getMatchDetail(leagueId, fixtureId)` + `MatchDetail`/`MatchTeamDetail`/`MatchPlayer`/`MatchScoreboard` types matching D3/D2 contract (`FixtureDraft` reuse, nullable `result`)
