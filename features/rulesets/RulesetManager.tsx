...
function RulesetCard({
  ...
  return (
    <li className="relative flex min-w-[260px] flex-1 snap-start flex-col rounded-xl border-[1.5px] border-slate-200 bg-panel p-3.5 transition-colors hover:border-navy">
      <div className="flex items-start justify-between gap-2">
        <button
          type="button"
          onClick={() => onEdit(ruleset)}
          className="text-left text-[15px] font-black text-navy hover:underline"
        >
          {ruleset.name}
        </button>
        <button
          type="button"
          onClick={() => onEdit(ruleset)}
          className="rounded-md border border-slate-200 bg-panel px-2 py-1 text-[11px] font-extrabold text-navy hover:border-navy"
        >
          {t("rulesets.edit")}
        </button>
      </div>
      ...
      <div className="mt-2.5 flex flex-wrap gap-1.5">
        <span className="rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">
          {t("rulesets.chip.races")} <b className="text-navy">{ruleset.races.length}/{RACES.length}</b>
        </span>
        <span className="rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">
          {t("rulesets.chip.treasury")} <b className="text-navy">{formatGold(ruleset.startingTreasury)}</b>
        </span>
        <span className="rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">
          {t("rulesets.chip.tv")} <b className="text-navy">{formatTvCap(ruleset.tvCap)}</b>
        </span>
        <span className="rounded-full border border-slate-200 bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">
          <b className="text-navy">{ruleset.minPlayers}–{ruleset.maxPlayers}</b> {t("rulesets.chip.players")}
        </span>
        <span
          className={`rounded-full border px-2 py-0.5 text-[11px] font-extrabold ${
            ruleset.active
              ? "border-success bg-success-fill text-success-strong"
              : "border-error-border bg-[#fee2e2] text-[#991b1b]"
          }`}
        >
      ...
      <div className="mt-auto flex items-center justify-end gap-2 pt-3">
        <button
          ...
          className="rounded-md border border-slate-300 px-2.5 py-1 text-[11px] font-bold text-slate-600 hover:border-navy hover:text-navy"
        >
        ...
        {confirmingDelete ? (
          <span className="flex items-center gap-1.5">
            ...
            <button
              type="button"
              onClick={remove}
              className="rounded-md bg-red px-2.5 py-1 text-[11px] font-bold text-white hover:bg-red-hover"
            >
              {t("common.yes")}
            </button>
            <button
          ...
          <button
            ...
            className="rounded-md border border-slate-300 px-2.5 py-1 text-[11px] font-bold text-slate-600 hover:border-red hover:text-red"
          >
            {t("rulesets.delete")}
          </button>
        )}
...
export function RulesetManager() {
  ...
  return (
    <section aria-labelledby="rulesets-heading">
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3 bg-navy px-4 py-[22px] text-white sm:px-6">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[13px] text-border-subtle">{t("rulesets.heroSubtitle")}</p>
        </div>
        <button
          type="button"
          onClick={openCreate}
          className="rounded-md border-2 border-red bg-red px-4 py-2 text-sm font-bold text-white transition-colors hover:bg-red-hover"
        >
      ...
      {editor ? (
        ...
        <div className="mt-8 border border-dashed border-slate-300 bg-panel px-6 py-10 text-center">
