      ...
      {resultFixture && resultMode === "correct" ? (
        <MatchActaWizardFor
          ...
          onSubmit={async (payload) => {
            // Only close on success: the wizard surfaces a rejection (e.g. a 409
            // race) in its alert and keeps itself open.
...
function MatchActaWizardFor({
      ...
      .then((match) => {
        ...
        if (mode === "correct") {
          if (match.result?.scores) {
            setInitial(actaPrefill(match.result.scores, match.result.weather));
          }
        } else if (match.live && match.live.status === "finished") {
  ...
  }, [fixture.leagueId, fixture.id, mode]);
