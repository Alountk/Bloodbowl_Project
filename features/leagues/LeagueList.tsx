...
export function StatusBadge({ status }: { status: League["status"] }) {
  ...
  if (status === "started") {
    return (
      <span className="rounded-full bg-navy px-2.5 py-0.5 text-[11px] font-bold text-white">
  ...
  if (status === "finished") {
    return (
      <span className="rounded-full bg-warning px-2.5 py-0.5 text-[11px] font-bold text-navy">
...
export function LeagueCard({ league }: { league: League }) {
  const { t } = useI18n();
  return (
    <li className="flex flex-col overflow-hidden rounded-none border border-slate-200 bg-panel">
      <div className="h-[6px] border-b-2 border-red bg-navy" />
      <div className="flex flex-1 flex-col p-4">
        <div className="flex items-start justify-between gap-2">
          <h3 className="text-[15px] font-extrabold text-navy">{league.name}</h3>
        ...
        <div className="mt-auto flex justify-end pt-3">
          <Link
            ...
            className="rounded-md border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-600 transition-colors hover:border-navy hover:text-navy"
          >
            {t("leagues.view")}
          </Link>
        </div>
    ...
    <section aria-labelledby="leagues-heading">
      ...
      <header className="mb-5 flex flex-wrap items-center justify-between gap-3 bg-navy px-4 py-[22px] text-white sm:px-6">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[13px] text-border-subtle">{t("leagues.heroSubtitle")}</p>
        </div>
        <button
          type="button"
      ...
      {loading ? null : error ? (
        ...
        <>
          <section aria-labelledby="my-leagues-heading" className="mb-8">
            ...
            {myLeagues.length === 0 ? (
              <p className="border border-slate-200 bg-panel p-6 text-center text-sm text-slate-600">
                {t("leagues.myEmpty")}
              </p>
            ) : (
