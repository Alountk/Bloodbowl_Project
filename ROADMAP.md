...
| **Registro de acciones player-first** (Diseño B aprobado, SDD `live-event-action-entry`): la tira de jugadores vivos reemplaza el FAB "+" — tocás al jugador y la burbuja ofrece sus acciones legales (TD/Pase en 2 toques con causante prefijado; baja/falta guiadas con stepper 1D16/1D6); **"ambos derribados"**: el coach no activo registra la caída del bloqueador rival (víctima rival + causante propio + marcador `bothDown`, acotado server-side), con ★2 simétrico en ambas action-cards (DEC-1) y ack payload-aware (el registrador nunca se auto-coteja; baja sin causante → badge, sin botones) | #161, #162, #163 |
| **Registro de acciones: dock contextual (Diseño A)** (SDD `live-match-ux-polish`): evolución v3 del registro — el dock fijo de acciones (TD/Pase/Baja/Falta activo; Baja propia/ambos derribados no-activo) reemplaza la tira player-first | #166 |
...
| **Crear equipo al apuntarse a una liga** (RAU-56): el wizard acepta el ruleset de la liga (razas filtradas, tesorería inicial, mín/máx, tope de TV) y `POST /api/teams` lo enforcea server-side (liga OPEN + guard RAU-54); el equipo se crea ya asignado con su base de tesorería (`Team.startingTreasury`, aditiva) | #146 |
